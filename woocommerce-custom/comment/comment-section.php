...
$starter_comment_quantity = get_option( 'page_comments', 0 ) ? get_option( 'comments_per_page', 2 ) : 0;
...
$starter_only_logged      = get_option( 'comment_registration' ) && ! is_user_logged_in();
$starter_only_verified    = ( 'yes' === get_option( 'woocommerce_review_rating_verification_required', 'no' ) && ! wc_customer_bought_product( '', get_current_user_id(), $starter_product_id ) );
?>
...
<div class="container mt-4" id="comments_wrap">
	...
	<?php if ( $starter_comment_count > $starter_comment_quantity && 0 != $starter_comment_quantity ) : ?>
		<div class="text-center">
			<a href="#" class="btn btn-outline-primary btn-lg js_comment_show_more" data-product_id="<?php echo esc_attr( $starter_product_id ); ?>">
				<span class="btn_static_txt"><?php esc_html_e( 'Show more', 'starter' ); ?></span>
	...
	<div id="write_comment">
		<?php if ( ! $starter_only_verified && $starter_only_logged ) : ?>
			<h4 class="text-center mt-5 pt-2"><?php esc_html_e( 'You must be logged in to post a review.', 'starter' ); ?></h4>
		<?php endif; ?>
		<?php if ( ! $starter_only_verified && ! $starter_only_logged ) : ?>
